...
function App() {
  ...
  const helloRef = useRef<SVGPathElement>(null);
  const iamRef = useRef<SVGPathElement>(null);
  const helloChangeDetect = setInterval(() => {
    const value = helloRef.current?.attributes[3].value;
    if (value === "rgba(255,255,255,0)") return;
    else {
      setVisible(false);
      clearInterval(helloChangeDetect);
    }
  }, 2000);
  const iamChangeDetect = setInterval(() => {
    const value = iamRef.current?.attributes[3].value;
    if (value === "rgba(255,255,255,0)") return;
    else {
      setVisibleTwo(false);
      clearInterval(iamChangeDetect);
    }
  }, 2000);
